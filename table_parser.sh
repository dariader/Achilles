...
#for((n = 3; n<10; n++))
 do
  ...
  printf '%s\n' $to_past > to_past.csv
  [ "$one" -eq "1" ] && paste resulting_dataframe.csv to_past.csv > resulting_dataframe_2.csv && rm resulting_dataframe.csv && mv resulting_dataframe_2.csv resulting_dataframe.csv ## append new column to res df
  done
